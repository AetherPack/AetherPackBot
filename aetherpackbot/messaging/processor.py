...
from aetherpackbot.protocols.messages import MessageChain
...
class WakeCheckStage(PipelineStage):
    ...
    async def process(self, context: PipelineContext) -> PipelineContext:
        ...
        # Private messages always wake
        if not message.session.is_group:
            context.is_wake = True
...
class AgentProcessingStage(PipelineStage):
    ...
    async def process(self, context: PipelineContext) -> PipelineContext:
        ...
        try:
            ...
            response = await orchestrator.process_message(context.event)
...
class ResponseDecorationStage(PipelineStage):
    ...
    async def process(self, context: PipelineContext) -> PipelineContext:
        ...
        if self._at_sender and context.event.message:
            from aetherpackbot.protocols.messages import MentionComponent
        ...
        if self._add_prefix and self._prefix_template:
            from aetherpackbot.protocols.messages import TextComponent
